...
def calculate_ph(dataset, varname, log):
    ...
    if not cc:
        ...
        cc = literal_eval(cc)
        pressure = dataset.pressure  # pressure in dbar
...
def calculate_ta(dataset, varname, log, configfile):
    ...
    try:
        ...
        configdict = cf.load_yaml_file(cc, logger=log)

        lat = np.nanmedian(dataset.profile_lat)
        lon = np.nanmedian(dataset.profile_lon)
        ...
        for region, values in configdict.items():
            if Polygon(list(zip(values['boundaries']['longitude'], values['boundaries']['latitude']))).contains(
                    Point(lon, lat)):
                log.debug(f'Segment within the {region} region')
                ...
                profile_ts = cf.convert_epoch_ts(np.nanmedian(dataset['profile_id']))
        ...
        if len(ta) == 0:
            log.debug(f'segment not within a defined region for estimating TA based on salinity: lat {lat}, lon {lon}')
...
def main(deployments, mode, loglevel, test):
    ...
    if isinstance(deployments_root, str):
        ...
        if not os.path.isdir(qc_config_derived):
            logging_base.warning(f'Invalid QC config path for derived variables: {qc_config_derived}')
        ...
        for deployment in [deployments]:  # for debugging
            ...
            if os.path.isfile(qctests_config_file):
                ...
                if not qctests_config_dict['add_derived_variables']:
                    logging.warning(f'Not calculating additional derived vars because test is turned off, check: {qctests_config_file}')
                    continue
            ...
            if not os.path.isfile(procvar_config_file):
                logging.error(f'Invalid science variable config file: {procvar_config_file}')
                continue

            proc_vars = cf.load_yaml_file(procvar_config_file, logger=logging)
                        ...
                        try:
                            ...
                            for qcf in qc_config_file_list:
                                ...
                                if not os.path.isfile(qc_config_file):
                                    logging.error(f'Invalid QC config file: {qc_config_file}')
                                    continue
