...
Last modified: lgarzio on 9/29/2025
Run ioos_qc QARTOD tests on processed glider NetCDF files and append the results to the original file.
The glider NetCDF files contain one glider segment and potentially multiple profiles.
"""
...
def build_global_regional_config(ds, qc_config_root):
    ...
    time = cf.convert_epoch_ts(np.nanmedian(ds['time']))
    lon = np.nanmedian(ds.longitude.values)
    lat = np.nanmedian(ds.latitude.values)
    ...
    region_bounds = cf.load_yaml_file(region_file)
    best_region = {'priority': 100, 'region': 'global'}

    # look for highest priority region containing the median lon and lat, with existing config
    ...
    if best_region['region'] != 'global':
        ...
        for c0 in c_region.config['contexts']:
            if c0['priority'] >= c['priority']:
                continue
            t0 = c0['window']['starting'].replace(year=time.year)
            t1 = c0['window']['ending'].replace(year=time.year)
            if np.logical_and(time >= t0, time <= t1):
                c = c0

    c['window']['starting'] = c['window']['starting'].replace(year=time.year)
    c['window']['ending'] = c['window']['ending'].replace(year=time.year)
        ...
        for deployment in [deployments]:  # for debugging
            ...
            for f in ncfiles:
                ...
                if not os.path.isdir(qc_config_root):
                    logging.warning(f'Invalid QC config root: {qc_config_root}')
                    return 1
                ...
                for inst in instruments:
                    ...
                    try:
                        ...
                        logging.error(f'Sensor maker and/or model not specified {inst}')
                    ...
                    if not os.path.isfile(qc_config_file):
                        logging.debug(f'Missing QC configuration file: {inst} {qc_config_file}')
                        continue
                ...
                pressure_rating = gsw.p_from_z(-depth_rating, np.nanmean(ds.latitude.values))
                ...
                attrs['comment'] = 'Glider depth rating (m) in flag_configurations converted to pressure (dbar) from ' \
                                   'pressure and latitude using gsw.p_from_z'
                if not hasattr(ds[sensor], 'ancillary_variables'):
                    ds[sensor].attrs['ancillary_variables'] = qc_varname
                else:
                    ds[sensor].attrs['ancillary_variables'] = ' '.join((ds[sensor].ancillary_variables, qc_varname))
                ...
                logging.debug(f'Using configuration file for region: {c['region']} and time window: {c['window']['starting'].strftime("%b-%d")} to {c['window']['ending'].strftime("%b-%d")}')
